...
export async function ShopPage({ locale }: { locale: Locale }) {
  ...
  return <main className="shop-page"><CommerceHeader locale={locale} />
    <section className="shop-hero"><div className="shop-hero-copy"><p className="eyebrow">Nene&apos;s little shop</p><h1>{content?.title || (en ? "Loved little things, ready for a new home." : "ของรักของเนเน่ พร้อมส่งต่ออย่างใส่ใจ")}</h1><p>{content?.body || (en ? "Thoughtfully selected essentials for babies and parents—with honest condition details and a story behind every item." : "สินค้าสำหรับแม่และเด็กที่คัดสรร ตรวจสอบสภาพ และบอกรายละเอียดอย่างตรงไปตรงมา เพื่อเริ่มต้นเรื่องราวบทใหม่กับครอบครัวของคุณ")}</p><div className="shop-hero-actions"><a href="#catalog"><Sparkles size={18} /> Shop Now</a><a href={en ? "/en/sell-with-nene" : "/sell-with-nene"}><HeartHandshake size={18} /> {en ? "Sell with Nene" : "ฝากขายกับเนเน่"}</a></div></div><div className="shop-hero-art"><div>🧸</div><span>🍼</span><i>♡</i><b>{en ? "Pre-loved with care" : "ส่งต่อด้วยความรัก"}</b></div></section>
